...
def render_unit(value: dict[str, Any], cidrs: list[str]) -> str:
    ...
    return "\n".join([
        ...
        "User=c5k4-harness",
        "Group=c5k4-harness",
        f"ExecStart={service['binary_path']} --activation-binding={service['activation_binding_path']} --expected-binding-sha256={binding} --tls-certificate={value['tls']['certificate_path']} --tls-private-key=%d/tls-private-key",
        f"LoadCredential=tls-private-key:{value['tls']['private_key_path']}",
        "WorkingDirectory=/var/lib/c5k4-harness",
        "UMask=0077",
        "NoNewPrivileges=yes",
        "PrivateDevices=yes",
        "PrivateTmp=yes",
        ...
        "ReadOnlyPaths=/opt/c5k4/p1 /etc/c5k4-harness",
        f"BindReadOnlyPaths={value['network']['resolution_artifact_path']}:/etc/hosts",
        "ReadWritePaths=/var/lib/c5k4-harness /run/c5k4-harness",
...
def generate(value: dict[str, Any], filesystem_root: Path = Path("/")) -> dict[str, Any]:
    ...
    bundle = {
        ...
        "unit": {"name": "c5k4-harness.service", "content": unit, "sha256": hashlib.sha256(unit.encode()).hexdigest()},
        "tls_material": {
