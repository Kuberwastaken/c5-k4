#!/usr/bin/env python3
"""Prepare and verify the immutable source/table gate for A056777 v2."""
from __future__ import annotations

import argparse, hashlib, json, math, os, pathlib, shutil

ROOT = pathlib.Path(__file__).resolve().parents[1]
HERE = ROOT / "results/expansion/live-search-2026-08-14/oeis-a056777-v2-development"
MANIFEST = HERE / "manifest.json"
M = json.loads(MANIFEST.read_text())
SCHEMA = "oeis-a056777-v2-gate-v1"
...
def self_hash(value: dict) -> str:
    body = dict(value); body.pop("attestation_sha256", None)
...
def parse_bfile(path: pathlib.Path) -> list[tuple[int, int]]:
    ...
    spec = M["oeis_bfile"]
    if len(rows) != spec["rows"] or [i for i, _ in rows] != list(range(1, len(rows)+1)) or list(rows[-1]) != [spec["last_index"], spec["last_value"]]:
...
def verify_sources(lean: pathlib.Path, source: pathlib.Path, bfile: pathlib.Path) -> dict:
    ...
    for token in ("%I A056777 #28 Jun 13 2026 17:40:34", "verified up to 10^12", "n = 1..166"):
        if token not in source_text: raise ValueError("OEIS statement drift")
    rows = parse_bfile(bfile); primes = primes_through(math.isqrt(rows[-1][1] + 12))
    prior = 0; stream = hashlib.sha256()
    for index, n in rows:
        if n <= prior: raise ValueError("b-file values not strictly increasing")
        fn, fm = factor(n, primes), factor(n+12, primes)
        if len(fn) != 2 or any(e != 1 for _, e in fn): raise ValueError("b-file non-semiprime")
        p, terminal = fn[0][0], fn[1][0]
        if terminal != p+8 or fm != [(p+2, 1), (p+6, 1)]: raise ValueError("b-file non-quadruple")
        phi, sigma = arithmetic(fn); phim, sigmam = arithmetic(fm)
        if phim != phi+12 or sigmam != sigma+12: raise ValueError("b-file equation drift")
        stream.update(f"{index},{n},{p},{phi},{phim},{sigma},{sigmam}\n".encode("ascii")); prior = n
...
def prepare(lean: pathlib.Path, source: pathlib.Path, bfile: pathlib.Path, output: pathlib.Path, commit: str) -> None:
    commit = exact_commit(commit); table = verify_sources(lean, source, bfile)
    ...
    value = {"schema": SCHEMA, "campaign_commit": commit, "manifest_sha256": sha(MANIFEST),
             ...
             "historical_exclusion_upper_inclusive": M["historical_exclusion_upper_inclusive"], "table": table,
             "snapshots": {name: sha(snapshots/name) for name in ("56777.lean", "A056777.seq", "b056777.txt")}}
    value["attestation_sha256"] = self_hash(value); atomic_json(output/"gate-attestation.json", value)


def verify(bundle: pathlib.Path, commit: str) -> dict:
    commit = exact_commit(commit); value = json.loads((bundle/"gate-attestation.json").read_text())
    keys = {"schema","campaign_commit","manifest_sha256","source_commit","historical_exclusion_upper_inclusive","table","snapshots","attestation_sha256"}
    if set(value) != keys or value["schema"] != SCHEMA or value["attestation_sha256"] != self_hash(value): raise ValueError("gate identity drift")
    if value["campaign_commit"] != commit or value["manifest_sha256"] != sha(MANIFEST) or value["source_commit"] != M["formal_conjectures"]["commit"]: raise ValueError("gate binding drift")
    snapshots = bundle/"snapshots"; actual = verify_sources(snapshots/"56777.lean", snapshots/"A056777.seq", snapshots/"b056777.txt")
    hashes = {name: sha(snapshots/name) for name in ("56777.lean", "A056777.seq", "b056777.txt")}
    if value["table"] != actual or value["snapshots"] != hashes or value["historical_exclusion_upper_inclusive"] != 10**12: raise ValueError("gate semantic drift")
    return value
