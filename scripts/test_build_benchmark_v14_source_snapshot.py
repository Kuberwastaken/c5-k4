#!/usr/bin/env python3
"""Tests for semantics-blind Method v1.4 source discovery and S0 acquisition."""
...
import build_benchmark_v14_source_snapshot as snapshot
...
def public_protocol(root: Path, policy: Path, contract_raw: bytes) -> tuple[Path, Path, str, str]:
    ...
    return repo, p0t_path, p0t, snapshot.PUBLIC_PROTOCOL_REMOTE


class SourceSnapshotTests(unittest.TestCase):
    def test_production_discovery_rejects_noncanonical_public_remote(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            fixture = Fixture(root)
            protocol, attestation_path, p0t, _ = public_protocol(
                root, fixture.policy, fixture.contract_raw()
            )
            with self.assertRaisesRegex(ValueError, "canonical protocol remote"):
                fixture.config(attestation_path, protocol, p0t, "file:///tmp/not-public.git")

    def test_git_helper_uses_absolute_binary_and_sanitized_environment(self) -> None:
        result = subprocess.CompletedProcess([], 0, b"", b"")
        with mock.patch.dict("os.environ", {
            "GIT_CONFIG_COUNT": "1", "GIT_CONFIG_KEY_0": "core.pager",
            "GIT_CONFIG_VALUE_0": "malicious", "ALL_PROXY": "socks5://127.0.0.1:9",
        }), mock.patch.object(snapshot.subprocess, "run", return_value=result) as run:
            snapshot.git(Path("/tmp/fixture"), "status")
        self.assertEqual(run.call_args.args[0][0], "/usr/bin/git")
        self.assertEqual(run.call_args.kwargs["env"], snapshot.SAFE_GIT_ENV)
        self.assertNotIn("GIT_CONFIG_COUNT", run.call_args.kwargs["env"])
        self.assertNotIn("ALL_PROXY", run.call_args.kwargs["env"])

    def test_offline_git_audit_rejects_alternates(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            repo = make_repo(Path(directory) / "repo")
            alternates = repo / ".git/objects/info/alternates"
            alternates.write_text("/tmp/foreign-objects\n", encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "alternates"):
                snapshot.assert_offline_git_repository(repo)

    def test_offline_git_audit_rejects_shallow_and_promisor_state(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            base = Path(directory)
            shallow_repo = make_repo(base / "shallow")
            (shallow_repo / ".git/shallow").write_text("0" * 40 + "\n", encoding="ascii")
            with self.assertRaisesRegex(ValueError, "shallow"):
                snapshot.assert_offline_git_repository(shallow_repo)

            promisor_repo = make_repo(base / "promisor")
            run_git(promisor_repo, "config", "remote.origin.promisor", "true")
            with self.assertRaisesRegex(ValueError, "promisor"):
                snapshot.assert_offline_git_repository(promisor_repo)

            partial_repo = make_repo(base / "partial")
            run_git(partial_repo, "config", "extensions.partialClone", "origin")
            with self.assertRaisesRegex(ValueError, "promisor"):
                snapshot.assert_offline_git_repository(partial_repo)

            marker_repo = make_repo(base / "marker")
            (marker_repo / ".git/objects/pack/pack-fixture.promisor").touch()
            with self.assertRaisesRegex(ValueError, "promisor object markers"):
                snapshot.assert_offline_git_repository(marker_repo)

    def test_hostile_local_fsmonitor_is_rejected_without_execution(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            base = Path(directory)
            ...
            with self.assertRaisesRegex(ValueError, "executable local configuration"):
                snapshot.assert_offline_git_repository(repo)
            self.assertFalse(marker.exists())

    def test_hostile_included_textconv_is_rejected_without_execution(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            base = Path(directory)
            repo = make_repo(base / "repo", {"fixture.txt": "fixture\n"})
            marker = base / "executed"
            helper = base / "textconv.sh"
            helper.write_text(f"#!/bin/sh\ntouch '{marker}'\ncat \"$1\"\n", encoding="utf-8")
            helper.chmod(0o755)
            included = base / "included.gitconfig"
            included.write_text(
                f"[diff \"hostile\"]\n\ttextconv = {helper}\n",
                encoding="utf-8",
            )
            run_git(repo, "config", "include.path", str(included))
    ...
    def test_production_discovery_and_s0_make_no_network_command(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            fixture = Fixture(root)
            ...
            original_run = snapshot.subprocess.run
            ...
            with mock.patch.object(snapshot.subprocess, "run", side_effect=guarded_run):
                ...
                snapshot.acquire_s0(
                    config_path, fixture.policy, attestation_path, protocol,
                    p0t, remote, "2026-08-13T10:00:01Z",
                )
    ...
    def test_policy_classifies_frozen_project_directory_fixture(self) -> None:
        policy_path = Path(__file__).resolve().parents[1] / "results/benchmark/v1.4-protocol/source-path-purpose-policy.json"
    ...
    def test_authoritative_discovery_contract_is_exact_invocation_bytes(self) -> None:
        ...
        contract = root / "results/benchmark/v1.4-protocol/source-discovery-contract.json"
        expected = snapshot.discovery_contract_bytes(
            Path("/Users/kuber.mehta/Projects"),
            Path("/home/ec2-user/.local/share/c5k4-v14-ai-chats-snapshot"),
            [
                "claude-local=claude:/home/ec2-user/.local/share/c5k4-v14-session-snapshot/claude:claude",
                "codex-local=codex:/home/ec2-user/.local/share/c5k4-v14-session-snapshot/codex:codex",
            ],
            [
                "c5-k4-github=/Users/kuber.mehta/Projects/c5-k4/"
                "results/benchmark/v1.4-s0/c5-k4-github-releases.json"
            ],
        )
