#!/usr/bin/env python3
"""Focused tests for the Method v1.4 isolated equal-budget runner."""
...
SCRIPT = Path(__file__).with_name("run_benchmark_v14_job.py")
SPEC = importlib.util.spec_from_file_location("benchmark_v14_runner", SCRIPT)
...
class BenchmarkV14RunnerTests(unittest.TestCase):
    def test_git_output_uses_offline_audit_absolute_binary_and_safe_environment(self) -> None:
        repo = Path("/tmp/v14-runner-fixture")
        completed = __import__("subprocess").CompletedProcess([], 0, "head\n", "")
        with mock.patch.object(RUNNER.source_snapshot, "assert_offline_git_repository") as audit, \
             mock.patch.object(RUNNER.subprocess, "run", return_value=completed) as run:
            self.assertEqual(RUNNER.git_output(repo, "rev-parse", "HEAD"), "head")
        audit.assert_called_once_with(repo)
        self.assertEqual(run.call_args.args[0][0], "/usr/bin/git")
        self.assertEqual(run.call_args.kwargs["env"], RUNNER.source_snapshot.SAFE_GIT_ENV)
    ...
    def setUp(self) -> None:
        ...
        self.manifest = {
            "schema_version": "c5k4-benchmark-1.4",
            "benchmark_id": "method-v1.4-test",
            "phase": "C1_SELECTED",
    ...
    def contract(self, mode: str, arm: str | None, count: int, cpu_budget: int) -> dict:
        ...
        return {
            "$schema": "schemas/benchmark-run-contract-v1.4.schema.json",
            "schema_version": "c5k4-benchmark-run-contract-1.4",
            "benchmark_version": "c5k4-benchmark-1.4",
    ...
    def test_process_environment_is_a_secret_free_fixed_allowlist(self) -> None:
        ...
        self.assertEqual(environment["C5K4_BENCHMARK_VERSION"], "1.4")
