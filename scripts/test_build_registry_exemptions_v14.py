#!/usr/bin/env python3
"""Tests for provenance-bound Method v1.4 registry exemptions."""
...
import build_registry_exemptions_v14 as exemptions
...
class Fixture:
    def __init__(self, root: Path) -> None:
        ...
        self.artifact = (
            json.dumps(
                {"schema_version": "c5k4-open-inventory-1.4", "declarations": []},
                sort_keys=True,
                indent=2,
            )
            + "\n"
            ...
            "outputs": [
                {
                    ...
                    "schema_version": "c5k4-open-inventory-1.4",
...
class RegistryExemptionTests(unittest.TestCase):
    def test_git_helper_requires_offline_audit_and_hardened_delegate(self) -> None:
        repo = Path("/tmp/v14-exemption-fixture")
        with mock.patch.object(exemptions.source_snapshot, "assert_offline_git_repository") as audit, \
             mock.patch.object(exemptions.source_snapshot, "git", return_value=b"ok") as hardened:
            self.assertEqual(exemptions.git(repo, "rev-parse", "HEAD"), b"ok")
        audit.assert_called_once_with(repo)
        hardened.assert_called_once_with(repo, "rev-parse", "HEAD")
    ...
    def test_mixed_prose_cannot_be_registered_as_machine_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            fixture = Fixture(Path(directory))
            prose = b'{"schema_version":"c5k4-open-inventory-1.4"}\nDiscussion follows.'
