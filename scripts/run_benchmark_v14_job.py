#!/usr/bin/env python3
"""Run one content-addressed Method v1.4 benchmark job.
...
CONTRACT_SCHEMA = ROOT / "schemas" / "benchmark-run-contract-v1.4.schema.json"
...
def validate_contract(
    ...
    if contract["benchmark_version"] != "c5k4-benchmark-1.4":
        raise ContractError("contract is not a Method v1.4 contract")
...
def git_output(repo_root: Path, *args: str) -> str | None:
    source_snapshot.assert_offline_git_repository(repo_root)
    result = subprocess.run(
        ["/usr/bin/git", *source_snapshot.SAFE_GIT_CONFIG_ARGS, "-C", str(repo_root), *args], text=True,
        capture_output=True, check=False, env=source_snapshot.SAFE_GIT_ENV,
    )
...
def default_manifest_linter(manifest_path: Path) -> Iterable[Any]:
    try:
        module = importlib.import_module("lint_benchmark_v14")
    except ModuleNotFoundError:
        module = importlib.import_module("scripts.lint_benchmark_v14")
...
def execute(
    ...
    if manifest.get("schema_version") != "c5k4-benchmark-1.4":
        raise ContractError("runner accepts only a Method v1.4 manifest")
    ...
    if args.require_git_ancestry:
        ...
        ancestry = subprocess.run(
            ["/usr/bin/git", *source_snapshot.SAFE_GIT_CONFIG_ARGS, "-C", str(repo_root), "merge-base", "--is-ancestor", c1_commit, head],
        ...
        if status:
            raise ContractError("v1.4 execution requires a clean, immutable campaign checkout")

    ledger_path = output_root / "runtime-ledger.jsonl"
    metadata = {
        "schema_version": "c5k4-benchmark-run-artifact-1.4",
