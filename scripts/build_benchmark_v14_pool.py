#!/usr/bin/env python3
"""Build the pinned, syntax-only Method v1.4 prototype inventory and pool.
...
selection. The output is a pre-P0 prototype, not a v1.4 freeze artifact.
"""
...
SCHEMA_INVENTORY = "c5k4-open-inventory-1.4-prototype"
SCHEMA_POOL = "c5k4-question-cluster-pool-1.4-prototype"
...
def git(repo: Path, *args: str) -> bytes:
    return source_snapshot.git(repo, *args)
...
def extract(
    ...
    source_snapshot.assert_offline_git_repository(repo)
    ...
    if enforce_pin and (commit != PINNED_COMMIT or tree != PINNED_TREE):
        raise ValueError("formal-conjectures commit/tree does not match the v1.4 prototype pin")
    ref = commit
    names = git(
...
def main() -> int:
    ...
    if rules.get("schema_version") != "c5k4-five-strata-classifier-1.4-prototype":
