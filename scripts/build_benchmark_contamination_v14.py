#!/usr/bin/env python3
"""Replay Method v1.4 identity exposure without reading target semantics.
...
import classify_benchmark_provenance_v14 as provenance
import build_benchmark_v14_source_snapshot as source_snapshot


SCHEMA = "c5k4-contamination-inventory-1.4-prototype"
CONFIG_SCHEMA = "c5k4-contamination-sources-1.4-prototype"
OVERLAY_SCHEMA = "c5k4-eligible-cluster-pool-1.4-prototype"
FEASIBILITY_SCHEMA = "c5k4-quota-feasibility-1.4-prototype"
...
def git(repo: Path, *args: str, input_bytes: bytes | None = None) -> bytes:
    return source_snapshot.git(repo, *args, input_bytes=input_bytes)
    ...
    for entry in entries:
        ...
        if state == "DELETED" and kind == "absent":
            ...
            if entry.get("type") == "SYMLINK":
                raw = os.readlink(filesystem_path).encode("utf-8", "surrogateescape")
            else:
...
def iter_tree(root: Path, source: dict[str, Any]) -> Iterator[dict[str, Any]]:
    ...
    snapshot = source.get("tree_snapshot")
    if not isinstance(snapshot, dict) or not isinstance(snapshot.get("entries"), list):
        raise ValueError("tree source lacks its exact S0 inventory")
    for expected in snapshot["entries"]:
        if not isinstance(expected, dict):
            raise ValueError("tree source has an invalid S0 entry")
        relative = expected.get("relative_path")
        if not isinstance(relative, str) or relative.startswith("/") or ".." in Path(relative).parts:
            raise ValueError("tree source has an unsafe S0 path")
        current, raw = source_snapshot._filesystem_entry_with_raw(root, relative)
        current["layer"] = "CURRENT_TREE"
        if current != expected:
            raise ValueError(f"tree source drift: {source['id']}:{relative}")
...
def iter_source(source: dict[str, Any], registry_units: dict[tuple[str, str, str, str], str]) -> Iterator[dict[str, Any]]:
    ...
    if kind == "git":
        ...
        raw = root.read_bytes()
        if len(raw) != source.get("byte_count") or sha256(raw) != source.get("corpus_sha256"):
            raise ValueError(f"release snapshot drift: {source['id']}")
        data = json.loads(raw.decode("utf-8", "strict"), parse_int=str)
...
def build(pool: dict[str, Any], config: dict[str, Any], exemptions: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    policy_path = Path(__file__).resolve().parents[1] / "results/benchmark/v1.4-protocol/provenance-policy.json"
