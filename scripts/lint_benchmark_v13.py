#!/usr/bin/env python3
"""Fail-closed validator for Method v1.3 benchmark manifests."""
...
try:
    from select_benchmark_v13 import ARTIFACT_KEYS, canonical_json, select as replay_selection
except ModuleNotFoundError:  # pragma: no cover
    from scripts.select_benchmark_v13 import ARTIFACT_KEYS, canonical_json, select as replay_selection

try:
    from score_benchmark_v13 import ScoreError, score_manifest as replay_score
except ModuleNotFoundError:  # pragma: no cover
    from scripts.score_benchmark_v13 import ScoreError, score_manifest as replay_score


SCHEMA_VERSION = "c5k4-benchmark-1.3"
SCHEMA = Path(__file__).parents[1] / "schemas" / "benchmark-v1.3.schema.json"
ZERO_SHA256 = "0" * 64
STRATA = (
...
def semantic_findings(manifest: dict[str, Any], manifest_path: Path) -> list[Finding]:
    ...
    for index, snapshot in enumerate(snapshots):
        ...
        if source is None:
            continue
        expected_schema = "c5k4-source-snapshot-S0-1.3" if index == 0 else "c5k4-source-snapshot-supplemental-1.3"
    ...
    if pool is not None:
        if pool.get("schema_version") != "c5k4-eligible-cluster-pool-1.3":
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.format == "json": print(json.dumps([asdict(row) for row in findings], indent=2))
        ...
        print(f"benchmark-v1.3 lint: {len(findings)} error(s)")
