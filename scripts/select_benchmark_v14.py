#!/usr/bin/env python3
"""Replay the Method v1.4 feasibility gate and select C1 from verified entropy.
...
SCHEMA_VERSION = "c5k4-benchmark-selection-1.4"
POOL_SCHEMA_VERSION = "c5k4-eligible-cluster-pool-1.4"
FEASIBILITY_SCHEMA_VERSION = "c5k4-quota-feasibility-1.4"
C0_SCHEMA_VERSION = "c5k4-c0-randomness-contract-1.4"
...
DOMAIN = b"c5-k4/method-v1.4/C1\x00"
STRATA = (
...
def validate_pool(pool: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(clusters):
        ...
        if row["eligible"] and row["stratum"] not in STRATA:
            raise ValueError(f"{where}.stratum is not a frozen Method v1.4 stratum")
        if not row["eligible"] and row["stratum"] is not None and row["stratum"] not in STRATA:
            raise ValueError(f"{where}.stratum must be null or a frozen Method v1.4 stratum")
    return clusters
