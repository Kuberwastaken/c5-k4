#!/usr/bin/env python3
"""Contract tests for the one Method v1.4 production registry build."""
...
INPUT_SCHEMA_PATH = ROOT / "schemas/benchmark-registry-input-v1.4.schema.json"
OUTPUT_SCHEMA_PATH = ROOT / "schemas/benchmark-registry-output-v1.4.schema.json"
CONTRACT_PATH = ROOT / "results/benchmark/v1.4-protocol/registry-build-invocation.json"
HEX64 = "a" * 64
...
def file_ref(name: str, authority: str = "FROZEN_PRODUCTION_INPUT") -> dict:
    return {
        ...
        "schema_version": f"c5k4-{name}-1.4",
        "authority": authority,
    }
...
def input_fixture() -> dict:
    return {
        "schema_version": "c5k4-registry-build-input-1.4",
...
ARTIFACTS = (
    ("open_inventory", "open-inventory.json", "c5k4-open-inventory-1.4"),
    ("question_cluster_pool", "question-cluster-pool.json", "c5k4-question-cluster-pool-1.4"),
    ("provenance_inventory", "provenance-inventory.json", "c5k4-provenance-inventory-1.4"),
    ("contamination_inventory", "contamination-inventory.json", "c5k4-contamination-inventory-1.4"),
    ("eligible_pool", "eligible-cluster-pool.json", "c5k4-eligible-cluster-pool-1.4"),
    ("quota_feasibility", "quota-feasibility.json", "c5k4-quota-feasibility-1.4"),
)
...
def output_fixture() -> dict:
    ...
    return {
        "schema_version": "c5k4-registry-build-output-1.4",
...
class RegistryContractTests(unittest.TestCase):
    ...
    def test_invocation_is_exact_create_exclusive_single_build(self) -> None:
        ...
        self.assertEqual(
            ...
            ["VERIFY_PUBLIC_P0T_ADVERTISEMENT", "SOLE_UPSTREAM_RESOLUTION_AND_MATERIALIZATION"],
        )
        self.assertEqual(
            network["preflight"]["allowed_calls"][0]["remote"],
            "https://github.com/Kuberwastaken/c5-k4.git",
        )
        self.assertEqual(
            network["preflight"]["subprocess_environment"],
            network["production_build"]["git_environment"],
        )
        self.assertNotIn("PATH", network["preflight"]["subprocess_environment"])
    ...
    def test_contract_paths_and_schema_ids_are_exact(self) -> None:
        ...
        self.assertEqual(self.input_schema["properties"]["schema_version"]["const"], "c5k4-registry-build-input-1.4")
        self.assertEqual(self.output_schema["properties"]["schema_version"]["const"], "c5k4-registry-build-output-1.4")
