#!/usr/bin/env python3
"""Assemble and validate non-circular Method v1.4 P0A/P0T artifacts."""
...
SCHEMA = ROOT / "schemas/benchmark-v1.4-p0.schema.json"
SCHEMA_VERSION = "c5k4-method-v1.4-p0-1.0"
CONFIG_VERSION = "c5k4-method-v1.4-p0-components-1.0"
REQUIRED_COMPONENTS = (
    "protocol_document",
...
PROTOTYPE_SEGMENT = "v1.4-prototype"
JSON_CONTRACTS = {
    "inheritance_manifest": (
        "c5k4-method-inheritance-1.4",
        {"schema_version", "artifact_status", "predecessor", "scientific_design", "sole_change", "versioned_execution_plumbing", "forbidden_changes"},
    ),
    "five_strata_classifier": (
        "c5k4-five-strata-classifier-1.4",
    ...
    "grouping_rule": (
        "c5k4-question-cluster-grouping-rule-1.4",
    ...
    "upstream_ref_rule": (
        "c5k4-upstream-ref-rule-1.4",
        {"schema_version", "repository", "subtree", "remote_ref", "resolution_event", "materialization_command_template", "materialized_repository_path", "materialized_ref", "fresh_repository_required", "bare_repository_required", "required_destination_ref_count", "fetch_head_permitted", "shallow_repository_permitted", "remotes_promisors_or_alternates_permitted", "connectivity_fsck_required", "required_object_format", "tree_rule", "retry_or_repin"},
    ),
    "quotas": (
        "c5k4-benchmark-quotas-1.4",
        {"schema_version", "sampling_unit", "selected_n", "quotas", "no_backfill"},
    ),
    "budget_rule": (
        "c5k4-benchmark-budget-rule-1.4",
    ...
    "target_data_audit_rule": (
        "c5k4-target-data-audit-rule-1.4",
...
def git(*args: str) -> bytes:
    ...
    return subprocess.run(
        ["/usr/bin/git", *source_snapshot.SAFE_GIT_CONFIG_ARGS, "-C", str(ROOT), *args], check=True,
        stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        env=source_snapshot.SAFE_GIT_ENV,
    ).stdout
...
def validate_audit_receipt(receipt_ref: dict[str, str], components: dict[str, Any]) -> None:
    ...
    if receipt["schema_version"] != "c5k4-method-v1.4-target-data-audit-receipt-1.0":
