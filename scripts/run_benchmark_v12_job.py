#!/usr/bin/env python3
"""Run one content-addressed Method v1.2 benchmark job.
...
CONTRACT_SCHEMA = ROOT / "schemas" / "benchmark-run-contract-v1.2.schema.json"
...
def validate_contract(
    ...
    if contract["benchmark_version"] != "c5k4-benchmark-1.2":
        raise ContractError("contract is not a Method v1.2 contract")
...
def default_manifest_linter(manifest_path: Path) -> Iterable[Any]:
    try:
        module = importlib.import_module("lint_benchmark_v12")
    except ModuleNotFoundError:
        module = importlib.import_module("scripts.lint_benchmark_v12")
...
def execute(
    ...
    if manifest.get("schema_version") != "c5k4-benchmark-1.2":
        raise ContractError("runner accepts only a Method v1.2 manifest")
    ...
    if args.require_git_ancestry:
        ...
        if status:
            raise ContractError("v1.2 execution requires a clean, immutable campaign checkout")

    ledger_path = output_root / "runtime-ledger.jsonl"
    metadata = {
        "schema_version": "c5k4-benchmark-run-artifact-1.2",
