#!/usr/bin/env python3
"""Contract tests for the one Method v1.3 production registry build."""
...
INPUT_SCHEMA_PATH = ROOT / "schemas/benchmark-registry-input-v1.3.schema.json"
OUTPUT_SCHEMA_PATH = ROOT / "schemas/benchmark-registry-output-v1.3.schema.json"
CONTRACT_PATH = ROOT / "results/benchmark/v1.3-protocol/registry-build-invocation.json"
HEX64 = "a" * 64
...
def file_ref(name: str, authority: str = "FROZEN_PRODUCTION_INPUT") -> dict:
    return {
        ...
        "schema_version": f"c5k4-{name}-1.3",
        "authority": authority,
    }
...
def input_fixture() -> dict:
    return {
        "schema_version": "c5k4-registry-build-input-1.3",
        ...
        "producer": {
            "producer_id": "method-v1.3-production-registry-builder",
            "executable_path": "scripts/build_benchmark_v13_registry.py",
...
ARTIFACTS = (
    ("open_inventory", "open-inventory.json", "c5k4-open-inventory-1.3"),
    ("question_cluster_pool", "question-cluster-pool.json", "c5k4-question-cluster-pool-1.3"),
    ("provenance_inventory", "provenance-inventory.json", "c5k4-provenance-inventory-1.3"),
    ("contamination_inventory", "contamination-inventory.json", "c5k4-contamination-inventory-1.3"),
    ("eligible_pool", "eligible-cluster-pool.json", "c5k4-eligible-cluster-pool-1.3"),
    ("quota_feasibility", "quota-feasibility.json", "c5k4-quota-feasibility-1.3"),
)
...
def output_fixture() -> dict:
    ...
    return {
        "schema_version": "c5k4-registry-build-output-1.3",
...
class RegistryContractTests(unittest.TestCase):
    ...
    def test_contract_paths_and_schema_ids_are_exact(self) -> None:
        ...
        self.assertEqual(self.input_schema["properties"]["schema_version"]["const"], "c5k4-registry-build-input-1.3")
        self.assertEqual(self.output_schema["properties"]["schema_version"]["const"], "c5k4-registry-build-output-1.3")
