...
v2 copy of ``scripts/gen/expressions.py``.  The template grammar is deliberately
**unchanged** from v1 -- small integer coefficients, sums and differences of two
invariants, ceilings and floors of halves, thirds and invariant/invariant ratios.
Only two things differ, both forced by the larger database and the larger
vocabulary:

  * ``SCALE`` is ``lcm(1..9) = 2520`` rather than ``lcm(1..8) = 840``, so that
    every ``Fraction``-valued invariant on a graph with ``n <= 9`` is an exact
    integer in SCALE units (``generate2.py`` asserts this graph by graph);
  * ``tree`` and ``path`` join the admissible denominators, both being ``>= 2``
    on every connected graph with ``n >= 2``.
...
from invariants2 import DISPLAY
...
# --------------------------------------------------------------------------
# Every Fraction-valued invariant on a graph with n <= 9 has a denominator
# dividing SCALE = lcm(1..9) = 2520:  deg_avg, ecc_avg, disp_avg and lam_avg have
# denominator n <= 9;  dist_avg has denominator n(n-1) <= 72 | 2520;  CW is a sum
# of 1/(1+d) with d <= 8, so its denominator divides lcm(1..9).  generate2.py
# asserts it for every graph rather than trusting the argument.
SCALE = 2520
