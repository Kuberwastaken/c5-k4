#!/usr/bin/env python3
"""Tests for isolated Method v1.4 C0A/C0T assembly."""
...
SCRIPT = Path(__file__).with_name("build_benchmark_v14_c0.py")
SPEC = importlib.util.spec_from_file_location("benchmark_v14_c0_tested", SCRIPT)
...
class C0Tests(unittest.TestCase):
    def test_git_helper_requires_offline_audit_and_hardened_delegate(self):
        with mock.patch.object(C0.source_snapshot, "assert_offline_git_repository") as audit, \
             mock.patch.object(C0.source_snapshot, "git", return_value=b"ok") as hardened:
            self.assertEqual(C0.git("rev-parse", "HEAD"), b"ok")
        audit.assert_called_once_with(C0.ROOT)
        hardened.assert_called_once_with(C0.ROOT, "rev-parse", "HEAD")

    def setUp(self):
        parent = C0.ROOT / "results" / "benchmark"
        self.temp = tempfile.TemporaryDirectory(prefix="v14-c0-test-", dir=parent)
        ...
        install("p0a", p0a)
        p0t = {
            "schema_version": C0.P0.SCHEMA_VERSION, "artifact_kind": "P0T", "protocol_version": "1.4",
