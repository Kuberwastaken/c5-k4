# Method v1.4 prospective benchmark (pre-P0 protocol)

**Status:** pre-P0 protocol scaffold. No v1.4 P0, S0, preflight, production
registry build, C0/C1, entropy, selection, or target-semantic inspection has
occurred.
**Purpose:** rerun the original twelve-cluster DeepMind comparison with the
identical v1.3 scientific design after correcting its upstream-object
materialization contradiction.

Method v1.4 is a prospective experiment. It does not reopen, extend, or
reinterpret Method v1.3. The v1.3 terminal result remains
`PROTOCOL_INVALID`.

## Sole change from v1.3

V1.4 inherits v1.3's sampling frame, five fixed strata, 3/3/2/2/2 quotas,
...
change. Its only correction replaces v1.3's upstream `ls-remote` with one
no-retry atomic fetch that both resolves `main` and materializes it into the
preregistered absent bare repository
`/home/ec2-user/.local/share/c5k4-v14-formal-conjectures.git`. The fetch writes
only `refs/c5k4-benchmark/v1.4/upstream-main`; it writes no `FETCH_HEAD`, remote,
shallow, promisor, or alternate state. The receipt binds the exact porcelain
output, commit, root tree, `FormalConjectures` subtree tree, and connectivity
audit for offline replay.

No eligibility rule, target population, classifier, quota, search budget,
forecast, transformation, scoring threshold, or retry policy changes in v1.4.
Any further change requires a later benchmark version.

V1.4 inherits v1.3's correction for the live-filesystem race that invalidated
v1.2's first invocation. After P0T, session synchronization is performed once and the exact
...
[`results/benchmark/v1.4-protocol/inheritance-manifest.json`](results/benchmark/v1.4-protocol/inheritance-manifest.json)
...
An actual v1.4 selection contains exactly twelve uncontaminated clusters from
...
unless a v1.4 freeze artifact explicitly restates an equivalent rule. New
solvers, target families, bounds, or backup transformations belong to a later
benchmark version.

## Two meanings of contact

V1.2 distinguishes two observably different kinds of identity contact. V1.4
...
cluster semantically exposed before v1.4 remains excluded.
...
before the one allowed v1.4 registry build and deterministically fixes the one
upstream commit that build may resolve.

After `P0T`, the operator acquires immutable snapshots of all semantic sources
through a recorded cutoff `S0`. No human or language model may inspect v1.4
...
[`scripts/build_benchmark_v14_source_snapshot.py`](scripts/build_benchmark_v14_source_snapshot.py).
Its path-purpose policy is
[`results/benchmark/v1.4-protocol/source-path-purpose-policy.json`](results/benchmark/v1.4-protocol/source-path-purpose-policy.json).
...
`c5k4-semantic-sources-config-1.4-prototype` object with
...
subtrees, ai-chats repository, and release exports). The P0T artifact does not
self-name its commit or remote: those are explicit acquisition arguments. The
tool verifies its exact committed bytes,
...
Public P0T advertisement is deliberately deferred to the first and only
advertisement network command in registry preflight. The second and final
preflight network command is the upstream materializing fetch. Source discovery,
S0 acquisition, and the production build perform no other network command.
The advertisement remote is exactly
`https://github.com/Kuberwastaken/c5-k4.git`; local paths, `file:` URLs,
redirected aliases, and alternate public URLs fail before any network command.
Every v1.4 Git subprocess uses `/usr/bin/git`, null global/system configuration,
disabled replacement and lazy-fetch behavior, noninteractive credentials, and
a fixed minimal environment. Source object stores with shallow, promisor, or
alternate-object state or executable local Git configuration fail closed.
Command-line overrides disable hooks, filesystem monitors, diff/textconv, and
clean/smudge processes; worktree diff discovery also passes `--no-ext-diff`
and `--no-textconv`. Unversioned-tree regular files and symlinks are scanned
from the same byte buffer whose size/hash/type was matched to S0, with symlinks
represented by their link-target bytes and never followed. Release JSON is
likewise parsed from the single byte buffer first matched to its S0 size/hash.
...
If the gate fails, v1.4 terminates immediately as
...
Only a passing certificate permits C0. Thus v1.4 can start the decisive
...
randomness-hash checks must pass. Selection uses the v1.4 domain-separated
...
clusters; its exact treatment must be fixed in the v1.4 scoring artifact at
...
Add `benchmark-v1.4.schema.json` rather than editing an earlier schema. The new
...
The v1.2 and v1.3 schemas, terminal artifacts, and validators remain
byte-for-byte immutable and independently replayable.

## Required linter and executable changes

Implement versioned v1.4 builders, selector, linter, scorer, and tests. The
v1.4 linter must fail closed unless it can prove all of the following:
...
No v1.4 C0 freeze is permitted until these checks are executable in CI and the
