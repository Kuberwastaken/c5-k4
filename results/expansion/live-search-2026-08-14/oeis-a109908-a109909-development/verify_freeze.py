...
if registry.get("schema")!="oeis-a109908-a109909-freeze-files-v1.1" or set(registry.get("sha256",{}))!=expected:raise SystemExit("freeze registry coverage drift")
...
if m.get("schema")!="oeis-a109908-a109909-development-freeze-v1.1" or m.get("development_only")is not True or m.get("target_evaluation_requires_exact_campaign_commit")is not True:raise SystemExit("manifest identity/lock drift")
if (m["internal_seconds"],m["external_search_seconds"],m["external_verify_seconds"])!=(48,54,60):raise SystemExit("cap drift")
if m.get("live_audit")!={"schema":"oeis-a109908-a109909-live-audit-v1.1","method":"parallel-rest-double-snapshot-v1","external_seconds":60,"open_pull_file_workers":16,"require_exact_changed_file_counts":True,"require_rename_previous_filenames":True,"require_stable_before_after_snapshot":True}:raise SystemExit("live-audit v1.1 contract drift")
...
if status.get("schema")!="oeis-a109908-a109909-source-status-v1.1" or status.get("execution_audit_method")!="parallel-rest-double-snapshot-v1":raise SystemExit("source audit supersession drift")
if "31835698077" not in status.get("execution_audit_supersession","") or status.get("local_project_commit")!=m["local_parent_commit"] or status.get("linked_public_proof_claim",{}).get("audit")!="PUBLIC_PROOF_CLAIM_AUDITED_INVALID" or status.get("fresh_race_check_required_at_execution")is not True:raise SystemExit("source/prior-art attestation drift")
...
for token in ("exact lowercase 40-hex campaign commit required","parallel-rest-double-snapshot-v1","open pull pagination/search-count mismatch","pagination truncated/raced","previous_filename","live source/status race detected","live source/race drift","live duplicate-search drift","ingestion/release race drift","Nat-safe symmetry failure","factorint(value)","frozen_profiles()","ordinal%M[\"shards\"]","signal.alarm(M[\"internal_seconds\"])","atomic_json(args.output/\"candidate.json\""):
...
for token in ("test_nat_safe_full_half_microfixtures","test_congruence_hypotheses_root_pairing_and_properness","test_lcm_obstruction","test_target_free_constructor_boundary","test_compact_cover_and_escape_replay_microfixtures","test_live_audit_search_timeout_and_incomplete_fail_closed","test_live_audit_pagination_omission_and_repetition_fail_closed","test_live_audit_rename_touch_and_snapshot_race","test_live_audit_schema_and_canonical_bytes_fail_closed"):
...
for token in ("PUBLIC_PROOF_CLAIM_AUDITED_INVALID","31835698077","parallel","previous_filename","gcd(k,q)=1","q=2","tighter even-`n` arm","Q=13,082,761,331,670,030","compact `candidate.json`","No target"):
