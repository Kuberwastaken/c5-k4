...
> `independence_number(x) <= minimum(size(x), 10^laplacian_energy(x))`
...
### graphbrain-alpha-upper-132 — HOLD_ARSENAL

> `independence_number(x) <= maximum(szekeres_wilf(x), 10^floor(lovasz_theta_over_root_n(x)))`

No violation was found on all 996 connected nonempty Graph Atlas graphs through order 7 where the exact author expression is defined, the named controls, or campaign and threshold families.  Real comparisons use the `1e-6` guard and ILP/LP runs stay within 60 seconds.  This is an arsenal hold, not a proof.


### graphbrain-alpha-upper-131 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= maximum(girth(x), min_degree(x))^laplacian_energy(x)`

The author project's current graph-object source explicitly supplies a counterexample to this exact formula.  The still-open issue entry is stale; this is a retro-kill and no novelty is claimed.


### graphbrain-alpha-upper-130 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= 2*wilf(x)^2 + radius(x)`

The author project's current graph-object source explicitly supplies a counterexample to this exact formula.  The still-open issue entry is stale; this is a retro-kill and no novelty is claimed.


### graphbrain-alpha-upper-129 — HOLD_ARSENAL

> `independence_number(x) <= e^(cosh(maximum(min_common_neighbors(x), lovasz_theta_over_root_n(x))))`

No violation was found on all 996 connected nonempty Graph Atlas graphs through order 7 where the exact author expression is defined, the named controls, or campaign and threshold families.  Real comparisons use the `1e-6` guard and ILP/LP runs stay within 60 seconds.  This is an arsenal hold, not a proof.


### graphbrain-alpha-upper-128 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= ceil(laplacian_energy(x)*max_degree(x))`

The author project's current graph-object source explicitly supplies a counterexample to this exact formula.  The still-open issue entry is stale; this is a retro-kill and no novelty is claimed.


### graphbrain-alpha-upper-127 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= minimum(fractional_alpha(x), 10^laplacian_energy(x))`

The author project's current graph-object source explicitly supplies a counterexample to this exact formula.  The still-open issue entry is stale; this is a retro-kill and no novelty is claimed.


### graphbrain-alpha-upper-126 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= minimum(cvetkovic(x), 10^laplacian_energy(x))`

The author project's current graph-object source explicitly supplies a counterexample to this exact formula.  The still-open issue entry is stale; this is a retro-kill and no novelty is claimed.


### graphbrain-alpha-upper-125 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= ceil(e^caro_wei(x))`

The author project's current graph-object source explicitly supplies a counterexample to this exact formula.  The still-open issue entry is stale; this is a retro-kill and no novelty is claimed.


### graphbrain-alpha-upper-124 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= ceil(e^inverse_degree(x))`

The author project's current graph-object source explicitly supplies a counterexample to this exact formula.  The still-open issue entry is stale; this is a retro-kill and no novelty is claimed.


### graphbrain-alpha-upper-123 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= minimum(annihilation_number(x), 10^laplacian_energy(x))`

The author project's current graph-object source explicitly supplies a counterexample to this exact formula.  The still-open issue entry is stale; this is a retro-kill and no novelty is claimed.


### graphbrain-alpha-upper-122 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= (diameter(x) + 1)*residue(x)`

The author project's current graph-object source explicitly supplies a counterexample to this exact formula.  The still-open issue entry is stale; this is a retro-kill and no novelty is claimed.


### graphbrain-alpha-upper-121 — RETRO_KILL_AUTHOR_SOURCE

> `independence_number(x) <= minimum(lovasz_theta(x), 10^laplacian_energy(x))`
...
### graphbrain-alpha-upper-118 — DB_REJECTED

> `independence_number(x) <= e^(max_common_neighbors(x)/density(x))/matching_number(x)`

The exact author reading fails the mandatory small-graph gate on connected Graph Atlas graph `atlas:EtaG`: `alpha=3` and right side `2.841252153680866`.  This is database-inconsistent as posted, not a new campaign result.  Undefined-domain cases were excluded.


### graphbrain-alpha-upper-117 — HOLD_ARSENAL

> `independence_number(x) <= cosh(degree_sum(x) - order_automorphism_group(x)) + girth(x)`

No violation was found on all 996 connected nonempty Graph Atlas graphs through order 7 where the exact author expression is defined, the named controls, or campaign and threshold families.  Real comparisons use the `1e-6` guard and ILP/LP runs stay within 60 seconds.  This is an arsenal hold, not a proof.


### graphbrain-alpha-upper-116 — DB_REJECTED

> `independence_number(x) <= size(x) + tan(number_of_triangles(x)/card_center(x))`

The exact author reading fails the mandatory small-graph gate on connected Graph Atlas graph `atlas:@`: `alpha=1` and right side `0`.  This is database-inconsistent as posted, not a new campaign result.  Undefined-domain cases were excluded.


### graphbrain-alpha-upper-115 — DB_REJECTED

> `independence_number(x) <= maximum(max_even_minus_even_horizontal(x), degree_sum(x)/sigma_2(x) - 1)`

The exact author reading fails the mandatory small-graph gate on connected Graph Atlas graph `atlas:FpUK_`: `alpha=4` and right side `3.5`.  This is database-inconsistent as posted, not a new campaign result.  Undefined-domain cases were excluded.
...
### graphbrain-alpha-upper-112 — DB_REJECTED

> `independence_number(x) <= maximum(card_periphery(x), size(x) - tan(average_distance(x)))`

The exact author reading fails the mandatory small-graph gate on connected Graph Atlas graph `atlas:E~_O`: `alpha=3` and right side `2`.  This is database-inconsistent as posted, not a new campaign result.  Undefined-domain cases were excluded.


### graphbrain-alpha-upper-111 — DB_REJECTED

> `independence_number(x) <= maximum(min_degree(x), order(x) - tan(number_of_triangles(x)))`

The exact author reading fails the mandatory small-graph gate on connected Graph Atlas graph `atlas:F}vUg`: `alpha=3` and right side `2`.  This is database-inconsistent as posted, not a new campaign result.  Undefined-domain cases were excluded.
