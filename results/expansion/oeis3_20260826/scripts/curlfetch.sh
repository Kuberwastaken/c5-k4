...
for i in $IDS; do
  A="A$(printf '%06d' $i)"
  [ -s "$BASE/oeis_pages/$A.html" ] || curl -sS --retry 3 -A "$UA" -o "$BASE/oeis_pages/$A.html" "https://oeis.org/$A/internal"
  echo "page $A $(wc -c < "$BASE/oeis_pages/$A.html" 2>/dev/null)"
  sleep 1
done
for i in $IDS; do
  A="A$(printf '%06d' $i)"
  [ -s "$BASE/bfiles/b$A.txt" ] || curl -sS --retry 3 -A "$UA" -o "$BASE/bfiles/b$A.txt" "https://oeis.org/$A/b$A.txt"
  if head -c 200 "$BASE/bfiles/b$A.txt" 2>/dev/null | grep -q "Table of"; then echo "bfile $A ok"; else echo "bfile $A MISSING"; fi
  sleep 1
done
