...
**Result: PASS, exit 0, warning-clean.** `✔ [8055/8055] Built FormalConjectures.OEIS.«108864»`.
Lean `v4.27.0`; mathlib oleans restored with `lake exe cache get` (7869 files), not built from
source.

### 8.1 Axiom audit

`lake env lean` over a scratch file importing the module:
...
Every `test` and `API` theorem stays inside the three standard axioms — no `decide +native`,
no `native_decide`, no `Lean.ofReduceBool` / `Lean.trustCompiler`. `conjecture` carries
`sorryAx` by design: it is `research open` and this submission does not close it.

### 8.2 Lean-side reproduction of the published data

Evaluated directly from the new `perfectDeficiency`, in the same session:

| Check | Result |
|---|---|
| A109883's first **79** published terms | `true` |
| A108864's **61** published DATA terms, regenerated by filtering `perfectDeficiency ≤ 10` | `true` |
| 0-indexed position of `1155` in the Lean-defined sequence | **58** — matches the `> 58` bound |
| `(perfectDeficiency 8925, perfectDeficiency 24, perfectDeficiency 56)` | `(2969, 0, 20)` |

The old predicate fails all four: it rejects `24`, accepts `56`, puts `1155` at index `52`,
and admits `8925`.

### 8.3 What was NOT verified

- Only `FormalConjectures.OEIS.«108864»` was built. The whole project was not built; CI will
  do that.
- The repo's own lint/CI jobs beyond `--wfail` (attribute extraction, `conjectures.json`
  regeneration) were not run locally.
- `a 58 = 1155` is **not** asserted as a Lean `test` theorem. `decide` on
  `Nat.count A 1155 = 58` would need the kernel to reduce `perfectDeficiency` on every
  `n < 1155`. It is verified by `#eval` (§8.2) and by two independent Python paths (§6), and
  the PR says so rather than claiming a kernel proof.
