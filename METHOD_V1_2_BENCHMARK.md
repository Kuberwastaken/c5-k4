...
An actual v1.2 selection contains exactly twelve uncontaminated clusters from
...
ledgers, and the rule against stopping after a crossing remain those of v1.1
unless a v1.2 freeze artifact explicitly restates an equivalent rule. New
solvers, target families, bounds, or backup transformations belong to a later
benchmark version.

## Two meanings of contact

V1.1 treated an identity-alias hit in an artifact as exposure. That conflated
two observably different events. V1.2 records both, but only one is semantic
contamination.
...
cluster semantically exposed before v1.2 remains excluded.
...
through a recorded cutoff `S0`. No human or language model may inspect v1.2
...
If the gate fails, v1.2 terminates immediately as
...
Only a passing certificate permits C0. Thus v1.2 can start the decisive
...
randomness-hash checks must pass. Selection uses the v1.2 domain-separated
...
clusters; its exact treatment must be fixed in the v1.2 scoring artifact at
...
Add `benchmark-v1.2.schema.json` rather than editing the v1.1 schema. The new
...
The v1.1 schema, terminal artifacts, and validator remain immutable and
independently replayable.

## Required linter and executable changes

Implement versioned v1.2 builders, selector, linter, scorer, and tests. The
v1.2 linter must fail closed unless it can prove all of the following:
...
No v1.2 C0 freeze is permitted until these checks are executable in CI and the
