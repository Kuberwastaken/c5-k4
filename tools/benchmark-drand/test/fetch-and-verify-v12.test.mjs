...
test('rejects a populated value, nonfuture close, or wrong legacy chain before I/O', () => {
  ...
  assert.throws(() => profileFromC0(populated), /must be null/)
  const nonfuture = c0()
  ...
  assert.throws(() => profileFromC0(nonfuture), /not future/)
  const wrongChain = c0()
  wrongChain.randomness.chain_hash = '0'.repeat(64)
  assert.throws(() => profileFromC0(wrongChain), /not legacy mainnet/)
})
