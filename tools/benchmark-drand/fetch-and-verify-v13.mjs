...
/** Offline-testable, C0-driven public-randomness retrieval for Method v1.3. */
...
export const C0_SCHEMA_VERSION = 'c5k4-c0-randomness-contract-1.3'
export const ARTIFACT_SCHEMA_VERSION = 'c5k4-drand-randomness-artifact-1'
export const INVOCATION_CONTRACT = Object.freeze({
  schema_version: 'c5k4-drand-fetch-invocation-contract-1.3',
...
export function profileFromC0 (c0, receipt, c0Raw) {
  ...
  if (receipt === null || typeof receipt !== 'object' || Array.isArray(receipt) || receipt.schema_version !== 'c5k4-c0-validation-receipt-1.3') throw new Error('valid C0 validation receipt is required')
...
function argumentsFrom (argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!['--c0-contract', '--c0-validation-receipt', '--output'].includes(key) || argv[index + 1] === undefined) throw new Error('usage: fetch-and-verify-v13.mjs --c0-contract FILE --c0-validation-receipt FILE --output FILE')
    values[key] = argv[index + 1]
  }
  if (!values['--c0-contract'] || !values['--c0-validation-receipt'] || !values['--output']) throw new Error('usage: fetch-and-verify-v13.mjs --c0-contract FILE --c0-validation-receipt FILE --output FILE')
  return values
}
...
async function main () {
  ...
  process.stderr.write(`verified v1.3 artifact written exclusively to ${output}\n`)
}

const invoked = process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)
if (invoked) {
  main().catch(error => {
    process.stderr.write(`v1.3 drand retrieval rejected: ${error.message}\n`)
    process.exitCode = 1
  })
}
